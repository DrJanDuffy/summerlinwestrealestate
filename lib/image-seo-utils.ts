...
export function generatePropertyAltText(
  data: ImageSEOData,
  imageType: 'exterior' | 'interior' | 'view' | 'amenity'
): string {
  const { location, propertyType, features, community, viewType, price, bedrooms, bathrooms } =
    data;
  ...
  switch (imageType) {
    case 'exterior': {
      ...
      const bedBathText =
        bedrooms && bathrooms ? ` with ${bedrooms} bedrooms and ${bathrooms} bathrooms` : '';
      const featureText =
        features.length > 0 ? ` featuring ${features.slice(0, 2).join(' and ')}` : '';
 ...
 */
export function generateCommunityAltText(
  community: string,
  imageType: 'entrance' | 'amenity' | 'landscape' | 'home'
): string {
  const baseLocation = 'Summerlin West, Las Vegas';
 ...
 */
export function generateMarketInsightAltText(
  topic: string,
  location: string = 'Summerlin West'
): string {
 ...
 */
export function generateAgentAltText(
  agentName: string,
  context: 'profile' | 'team' | 'action'
): string {
  const baseLocation = 'Summerlin West, Las Vegas';
 ...
 */
export function generateLifestyleAltText(
  activity: string,
  location: string = 'Summerlin West'
): string {
  const baseLocation = 'Las Vegas, Nevada';
