...
export default function SubdivisionPage({ params }: { params: { slug: string } }) {
  const { slug } = params;
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  // Memoize subdivision lookup with error handling
  const subdivision = useMemo(() => {
    try {
      return subdivisions.find((s) => s.slug === slug);
    } catch (err) {
      setError('Failed to load subdivision data');
      return null;
    }
  }, [slug]);
  ...
  const heroImage = (subdivision as Subdivision).image || "https://placehold.co/600x300?text=The+Vistas";
  const imageAlt = (subdivision as Subdivision).image 
