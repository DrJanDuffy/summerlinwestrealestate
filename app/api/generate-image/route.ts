...
export async function POST(req: NextRequest) {
  try {
    ...
    const imageDescription = data.choices[0]?.message?.content || 'Image generation failed';
    ...
    return NextResponse.json({ 
      ...
      placeholder: true,
      message: 'Perplexity provides image descriptions. For actual image generation, consider using a dedicated image service.'
    });
