...
async function testDomain(domain) {
  ...
  for (const page of testPages) {
    ...
    if (result.error) {
      ...
      console.log(
        `${formatStatus(result.status)} ${formatTime(result.responseTime)} ${result.contentLength} bytes`
      );
...
async function compareResults(allResults) {
  ...
  for (const page of testPages) {
    const pageResults = allResults.map((domainResult) => domainResult.find((r) => r.page === page));
  ...
  if (issues.length === 0) {
    console.log(
      `${colors.green}✓ No issues found - all domains serving consistent content${colors.reset}`
    );
  } else {
...
async function main() {
  ...
  console.log(
    `${colors.blue}Testing ${domains.length} domains across ${testPages.length} pages${colors.reset}`
  );
  ...
  console.log(
    `${colors.blue}Check the results above for any domain inconsistencies.${colors.reset}`
  );
}
