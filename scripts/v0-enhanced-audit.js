  ...
  return new Promise(async (resolve) => {
    ...
    resolve({
      ...
      isLoop: chain.some((step, index) =>
        chain.slice(index + 1).some((laterStep) => step.url === laterStep.url)
      ),
    });
  });
}
...
async function testDomainConfiguration() {
  ...
  for (const url of testUrls) {
    ...
    console.log(
      `  Loop Detected: ${analysis.isLoop ? colors.red + 'YES' + colors.reset : colors.green + 'NO' + colors.reset}`
    );
    ...
    if (analysis.chain.length > 1) {
      ...
      analysis.chain.forEach((step, index) => {
        console.log(
          `    ${index + 1}. ${step.url} → ${formatStatus(step.status)} ${step.location || ''}`
        );
      });
    }
  }
  ...
  for (const url of testUrls) {
    ...
    if (result.hasContent) {
      ...
      console.log(
        `    Has Title: ${hasTitle ? colors.green + '✓' + colors.reset : colors.red + '✗' + colors.reset}`
      );
      console.log(
        `    Has Summerlin: ${hasSummerlin ? colors.green + '✓' + colors.reset : colors.red + '✗' + colors.reset}`
      );
      console.log(
        `    Has Dr. Jan: ${hasDrJan ? colors.green + '✓' + colors.reset : colors.red + '✗' + colors.reset}`
      );
    }
  }
