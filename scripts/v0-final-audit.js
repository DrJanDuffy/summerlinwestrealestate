...
async function generateFinalAuditReport() {
  ...
  if (
    results['www.summerlinwestrealestate.com']['/']?.location?.includes(
      'summerlinwestrealestate.com'
    )
  ) {
    issues.push({
      type: 'CRITICAL',
      description:
        'www.summerlinwestrealestate.com redirects to summerlinwestrealestate.com (should be opposite)',
  ...
  const workingDomains = Object.keys(results).filter(
    (domain) => results[domain]['/']?.status === 200 || results[domain]['/']?.status === 307
  );
