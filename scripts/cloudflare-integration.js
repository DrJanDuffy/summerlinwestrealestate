...
function displaySecuritySettings() {
  ...
  log(
    `Always Use HTTPS: ${ssl.edge_certificates.always_use_https ? 'Enabled' : 'Disabled'}`,
    'green'
  );
  ...
  log(
    `Bot Fight Mode: ${config.security_settings.bot_fight_mode ? 'Enabled' : 'Disabled'}`,
    'green'
  );
  log(
    `Browser Integrity Check: ${config.security_settings.browser_integrity_check ? 'Enabled' : 'Disabled'}`,
    'green'
  );
}
...
function displayRealEstateOptimizations() {
  ...
  log(
    `WebP Conversion: ${optimizations.property_images.webp_conversion ? 'Enabled' : 'Disabled'}`,
    'green'
  );

  log(`\nMarket Data Cache: ${optimizations.market_data.cache_ttl}s`, 'green');
  log(
    `Market Data Stale While Revalidate: ${optimizations.market_data.stale_while_revalidate}s`,
    'green'
  );
