...
const config = {
  ...
  expectedHeaders: ['cf-ray', 'cf-cache-status', 'cf-request-id', 'server'],
};
...
async function testCacheEffectiveness() {
  ...
  try {
    ...
    log(
      `First Request Cache Status: ${firstRequest.headers['cf-cache-status'] || 'unknown'}`,
      'blue'
    );
    ...
    log(
      `Second Request Cache Status: ${secondRequest.headers['cf-cache-status'] || 'unknown'}`,
      'blue'
    );
...
async function generateReport(results) {
  ...
  log(
    `   Cloudflare Active: ${cloudflareActive}/${totalTests}`,
    cloudflareActive === totalTests ? 'green' : 'yellow'
  );
  ...
  results.forEach((result, index) => {
    if (result.error) {
      ...
      log(
        `   ${index + 1}. ${result.url}: ${status} ${result.responseTime?.toFixed(2)}ms`,
