...
export default function HomebotWidget() {
  ...
  useEffect(() => {
    ...
    if (!shadow.querySelector('script[src="' + HOMEBOT_SCRIPT_SRC + '"]')) {
      ...
      script.onload = () => {
        if (window.Homebot) {
          window.Homebot('#homebot_homeowner', HOMEBOT_WIDGET_KEY, { theme: 'dark-mode-theme' });
        }
      };
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', minHeight: 480, zIndex: 1000, borderRadius: 8, overflow: 'hidden' }} />
  );
} 