  ...
  useEffect(() => {
    const checkRealScout = () => {
      ...
      if (customElements.get('realscout-office-listings')) {
        ...
        console.log('RealScout custom elements not yet available, waiting...');
        // Wait a bit more for the script to load
        setTimeout(() => {
          if (customElements.get('realscout-office-listings')) {
            setIsLoaded(true);
          } else {
            console.log('RealScout custom elements still not available after timeout');
            setHasError(true);
          }
        }, 3000);
      }
    ...
    const timeoutId = setTimeout(checkRealScout, 1000);
