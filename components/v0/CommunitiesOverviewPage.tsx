...
export default function CommunitiesOverviewPage() {
  useEffect(() => {
    ...
    const schema = {
      ...
      description: 'Explore luxury communities in Summerlin West with amenities, schools, and market data',
      itemListElement: [
  ...
  const communities = [
    {
      name: 'The Vistas',
      description: 'Exclusive gated community with Red Rock Canyon views and resort-style amenities',
            ...
            <div className="relative bg-white rounded-2xl shadow-xl overflow-hidden">
              ...
              <div className="relative p-12">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <h3 className="text-4xl font-bold text-gray-900 mb-4">{communities.find(c => c.featured)?.name}</h3>
                    <p className="text-xl text-gray-700 mb-6">{communities.find(c => c.featured)?.description}</p>
                    <div className="space-y-4 mb-8">
                      <div className="flex items-center">
                        <svg className="w-6 h-6 text-blue-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                        </svg>
                        <span className="text-gray-700 font-semibold">{communities.find(c => c.featured)?.homes}</span>
                      </div>
                      <div className="flex items-center">
                        <svg className="w-6 h-6 text-blue-600 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span className="text-gray-700 font-semibold">{communities.find(c => c.featured)?.priceRange}</span>
                      </div>
                    </div>
                    <Link
                  ...
                  <div className={`${communities.find(c => c.featured)?.image} rounded-lg h-64`}></div>
                </div>
              </div>
            </div>
          )}
